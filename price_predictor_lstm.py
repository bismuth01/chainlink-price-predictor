...
lstm_next_price = predict_next_price(df_processed, model, SEQ_LEN, scaler, y_scaler)
...
lstm_predictions = [y_pred[i][0] for i in historical_indices] + [lstm_next_price]

print(f"Predicted price at {next_timestamp}: -")
print(f"LSTM: ${lstm_next_price:.2f}\n")
